...
def sendcmd(client,val):
    client.publish("aebsubscribe1",str(val))

def sendalert(client,msg):
    print('mobile alert')
    msgdata = "{\"title\":\"Help\" , \"body\":\"" + msg + "\"}"
    print(msgdata)
    client.publish("handgesture",msgdata)
...
while True:
    ...
    if className=="A":
        ...
        thread = threading.Thread(target=sendalert,args=(client,"Please send water"))
        print('mobile alert')
        thread.start()
        ...
        thread = threading.Thread(target=sendalert,args=(client,"Emergency...."))
        print('mobile alert')
        thread.start()
